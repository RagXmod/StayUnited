...
export default class Category {
    ...
    loadTables() {
        ...
        jQuery(this.domIds.page_table).DataTable({
            ...
            serverSide: true,
            stateSave: true,
            ajax: window.dcmUri['resource'] + '?parent_id=' + (window.dcmUri['parent_id'] || ''),
    ...
    confirmDeletion() {
        ...
        jQuery(this.domIds.confirm_del ).on('click', function() {
            if ( that.itemId ) {
                // request delete here..
                axios.delete( window.dcmUri['resource'] + '/' + that.itemId + '?parent_id=' + (window.dcmUri['parent_id'] || ''))
                .then( data => {
...
jQuery(() => {
   ...
   window.dcmCategory = new Category();
